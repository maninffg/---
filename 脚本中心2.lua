...
local Announce_Section_Button = Instance.new("TextButton")
...
local Announce_Section = Instance.new("ScrollingFrame")
...
Background.ZIndex = 9
Background.Image = "rbxassetid://159991693" -- 这里更改图标ID
...
TitleBarLabel.Text = "____//脚本中心2\\___" -- 这里更改标题名字
...
Announce_Section_Button.Name = "Announce_Section_Button"
Announce_Section_Button.Parent = SectionList
Announce_Section_Button.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
Announce_Section_Button.BackgroundTransparency = 0.500
Announce_Section_Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
Announce_Section_Button.BorderSizePixel = 0
Announce_Section_Button.Position = UDim2.new(0, 0, 0, 25)
Announce_Section_Button.Size = UDim2.new(0, 105, 0, 30)
Announce_Section_Button.Font = Enum.Font.Oswald
Announce_Section_Button.Text = "公告" -- 更更改选项文本
Announce_Section_Button.TextColor3 = Color3.fromRGB(0, 0, 0)
Announce_Section_Button.TextScaled = true
Announce_Section_Button.TextSize = 14.000
Announce_Section_Button.TextWrapped = true
...
Character_Section_Button.Text = "自身"
...
Animations_Section_Button.Text = "动画"
...
Misc_Section_Button.Text = "其他" -- 更改选项文本到此结束
...
Announce_Section.Name = "Announce_Section"
Announce_Section.Parent = Background
Announce_Section.Active = true
Announce_Section.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
Announce_Section.BackgroundTransparency = 1.000
Announce_Section.BorderColor3 = Color3.fromRGB(0, 0, 0)
Announce_Section.BorderSizePixel = 0
Announce_Section.Position = UDim2.new(0, 105, 0, 30)
Announce_Section.Size = UDim2.new(0, 395, 0, 320)
Announce_Section.CanvasSize = UDim2.new(0, 0, 1, 0)
Announce_Section.ScrollBarThickness = 5

Announce_Label.Name = "Announce_Label"
Announce_Label.Parent = Announce_Section
...
Announce_Label.Position = UDim2.new(0, 25, 0, 25)
Announce_Label.Size = UDim2.new(0, 350, 0, 270)
Announce_Label.Font = Enum.Font.SourceSans
Announce_Label.Text = "公告:欢迎使用脚本中心2,代码完全开放/开源,免费给你们代码,首先声明禁止圈钱,可二创,更新时间2026/1/16,发布时间2026/1/13,by(作者):4c9c304f4e9ad57r，如果想关闭此UI就去找一个键盘来。" --这里更改公告文本
Announce_Label.TextColor3 = Color3.fromRGB(0, 255, 255)
Announce_Label.TextSize = 18.000
...
Home_Section.CanvasSize = UDim2.new(0, 0, 1, 0)
...
Welcome_Label.Text = ("你好 @"..plr.Name.."!\n按[B]键打开/关闭界面.")
...
WalkSpeed_Input.PlaceholderText = "数字[1-99999]" -- 这里调节速度数字最大值0~无限
...
ClearCheckpoint_Button.Text = "清除检查点"
...
JumpPower_Input.PlaceholderText = "数字[1-99999]" -- 这里更改跳跃数字调节0~无限
...
SaveCheckpoint_Button.Text = "保存检查点"
...
FlySpeed_Input.PlaceholderText = "数字[1-99999]" -- 这里飞行速度调节0~无限
FlySpeed_Input.Text = ""
...
TargetName_Input.PlaceholderText = "@目标..."
...
WhitelistTarget_Button.Text = "白名单"
...
ZombieClassicAnim_Button.Text = "经典僵尸"
...
AntiChatSpy_Button.Text = "防聊天被监控"
...
ChatBox_Input.PlaceholderText = "聊天绕过[您的消息不会被封禁]"
ChatBox_Input.Text = ""
...
local function UpdateTarget(player)
	pcall(function()
		if table.find(ForceWhitelist,player.UserId) then
			SendNotify("脚本中心2","你无法选择此玩家: @"..player.Name.." / "..player.DisplayName,5)
			player = nil
		end
	end)
	if (player ~= nil) then
...
ChangeSection(Announce_Section_Button)
Announce_Section_Button.MouseButton1Click:Connect(function()
	ChangeSection(Announce_Section_Button)
end)
...
WhitelistTarget_Button.MouseButton1Click:Connect(function()
	if TargetedPlayer ~= nil then
		if table.find(ScriptWhitelist, Players[TargetedPlayer].UserId) then
			...
			SendNotify("脚本中心2",TargetedPlayer.." 已从白名单移除.",5)
		else
			table.insert(ScriptWhitelist, Players[TargetedPlayer].UserId)
			SendNotify("脚本中心2",TargetedPlayer.." 已添加到白名单.", 5)
		end
	end
end)
...
Players.PlayerRemoving:Connect(function(player)
	pcall(function()
		if player.Name == TargetedPlayer then
			UpdateTarget(nil)
			SendNotify("脚本中心2","目标玩家已离开/重新加入.",5)
		end
	end)
end)
...
plr.CharacterAdded:Connect(function(x)
	...
	if Fly_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		...
		SendNotify("脚本中心2","飞行已自动关闭，因为您的角色重生",5)
	end
	...
	if ammount == original_ammount then
		...
		if not (count == original_ammount) then
			SysBroker:Destroy()
			SendNotify("脚本中心2","发生意外错误，正在重新加入...")
			game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId, plr)
		end
	else
		SysBroker:Destroy()
