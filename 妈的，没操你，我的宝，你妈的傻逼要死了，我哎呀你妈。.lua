...
do
    ...
    function CustomImageManager.AddAsset(
        ...
        assert(typeof(RobloxAssetId) == "number", "RobloxAssetId 必须是一个数字")
...
local Library = {
    ...
    DPIScale = 1,
    CornerRadius = 8, -- 增加圆角半径
    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(15, 15, 15),
        MainColor = Color3.fromRGB(25, 25, 25),
        AccentColor = Color3.fromRGB(125, 85, 255),
        OutlineColor = Color3.new(1, 1, 1), -- 改为白色
        FontColor = Color3.new(1, 1, 1),
        Font = Font.fromEnum(Enum.Font.Code),
        RedColor = Color3.fromRGB(255, 50, 50),
        DarkColor = Color3.new(0.1, 0.1, 0.1),
...
local Templates = {
    ...
    Window = {
        ...
        NotifySide = "Right",
        ShowCustomCursor = true,
        Font = Enum.Font.GothamSemibold, -- 使用更美观的字体
        ToggleKeybind = Enum.KeyCode.RightControl,
...
local function CreateRainbowGradient()
    ...
    gradient.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(0.14, Color3.fromRGB(255, 127, 0)), -- 橙色
        ColorSequenceKeypoint.new(0.28, Color3.fromRGB(255, 255, 0)), -- 黄色
        ColorSequenceKeypoint.new(0.42, Color3.fromRGB(0, 255, 0)),   -- 绿色
        ColorSequenceKeypoint.new(0.57, Color3.fromRGB(0, 255, 255)), -- 青色
        ColorSequenceKeypoint.new(0.71, Color3.fromRGB(0, 0, 255)),   -- 蓝色
        ColorSequenceKeypoint.new(0.85, Color3.fromRGB(139, 0, 255)), -- 紫色
        ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 0, 0))       -- 回到红色
    })
    gradient.Rotation = 0
    return gradient
end
...
--// 创建彩虹渐变动画 \\--
local function AnimateRainbowGradient(gradient)
...
local function GetSchemeValue(Index)
    ...
    if AliasIndex and Library.Scheme[AliasIndex] ~= nil then
        warn(string.format("方案值 %q 已过时，请改用 %q。", Index, AliasIndex))
...
local function IsDragInput(Input: InputObject, IncludeM2: boolean?)
    return IsMouseInput(Input, IncludeM2) and (Input.UserInputState == Enum.UserInputState.Begin or Input.UserInputState == Enum.UserInputState.Change)
...
function Library:ChangeCursorIcon(ImageId: string)
    ...
    assert(Icon, "图片必须是有效的Roblox资源或有效的URL或有效的lucide图标。")
...
function Library:ChangeCursorIconSize(Size: UDim2)
    assert(typeof(Size) == "UDim2", "期望UDim2。")
...
function Library:AddOutline(Frame: GuiObject)
    ...
    AnimateRainbowGradient(rainbowGradient)
    ...
    return RainbowStroke, ShadowStroke
end
...
function Library:MakeOutline(Frame: GuiObject, Corner: number?, ZIndex: number?)
    warn("Obsidian:MakeOutline 已过时，请改用 Obsidian:AddOutline。")
...
do
    ...
    function Library:SetWatermark(Text: string)
        warn("水印已过时，请改用 Library:AddDraggableLabel。")
...
function Library:AddTooltip(InfoStr: string, DisabledInfoStr: string, HoverInstance: GuiObject)
    ...
    local function DoHover()
        if CurrentHoverInstance == HoverInstance
            ...
            or (not TooltipTable.Disabled and typeof(InfoStr) ~= "string") then
            return
        end
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        if KeyPicker.Mode == "Press" then
            assert(ParentObj.Type == "标签", "模式为'按下'的按键选择器只能应用于标签。")
        ...
        KeyPicker.Modifiers = VerifyModifiers(KeyPicker.Modifiers) -- 验证默认修饰键
        ...
        function KeyPicker:SetValue(Data)
            ...
            KeyPicker.DisplayValue = if GetTableSize(KeyPicker.Modifiers) > 0 then
                (table.concat(KeyPicker.Modifiers, " + ") .. " + " .. KeyPicker.Value)
            else
                KeyPicker.Value
        ...
        Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject)
            ...
            if KeyPicker.Mode == "始终" or KeyPicker.Value == "未知" or KeyPicker.Value == "无" or Picking or UserInputService:GetFocusedTextBox() then
                return
            end
...
do
        ...
        local function InitEvents(Button)
            ...
            Button.Base.MouseButton1Click:Connect(function()
                ...
                if Button.DoubleClick then
                    ...
                    RunService.RenderStepped:Wait() --// 鼠标按钮不等待就触发（我讨厌Roblox）
                    Button.Locked = false
                    return
                end
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:SetMax(Value)
            ...
            Slider:SetValue(math.clamp(Slider.Value, Slider.Min, Value)) -- 这将使其更新
            Slider.Max = Value
            Slider:Display()
        end
        ...
        function Slider:SetMin(Value)
            ...
            Slider:SetValue(math.clamp(Slider.Value, Value, Slider.Max)) -- 同样
            Slider.Min = Value
            Slider:Display()
        end
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:SetupDependencies(Dependencies)
            for _, Dependency in Dependencies do
                assert(typeof(Dependency) == "table", "依赖项应该是表格。")
                assert(Dependency[1] ~= nil, "依赖项缺少元素。")
                assert(Dependency[2] ~= nil, "依赖项缺少预期值。")
            end
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        -- 为标题添加彩虹渐变
        local titleGradient = CreateRainbowGradient()
        titleGradient.Parent = WindowTitle
        AnimateRainbowGradient(titleGradient)
        ...
        -- 为标签标题添加彩虹渐变
        local tabLabelGradient = CreateRainbowGradient()
        tabLabelGradient.Parent = CurrentTabLabel
        AnimateRainbowGradient(tabLabelGradient)
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                -- 为组框标题添加彩虹渐变
                local groupboxGradient = CreateRainbowGradient()
                groupboxGradient.Parent = GroupboxLabel
                AnimateRainbowGradient(groupboxGradient)
...
local function OnPlayerChange()
    ...
    for _, Dropdown in Options do
        if Dropdown.Type == "下拉框" and Dropdown.SpecialType == "玩家" then
...
local function OnTeamChange()
    ...
    for _, Dropdown in Options do
        if Dropdown.Type == "下拉框" and Dropdown.SpecialType == "队伍" then
