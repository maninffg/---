...
local Library = { 
    ...
    Scheme = { 
        ...
        ButtonBorderColor = Color3.new(1, 1, 1), -- 白色按钮边框
    }, 
    Registry = {}, 
    Scales = {}, 
...
function Library:ChangeCursorIconSize(Size: UDim2) 
    assert(typeof(Size) == "UDim2", "应为UDim2。") 
...
do 
    ...
    function Library:SetWatermark(Text: string) 
        warn("水印已弃用，请使用Library:AddDraggableLabel代替。") 
...
-- 创建彩色边框函数
function Library:CreateColorfulOutline(Frame: GuiObject)
    local OutlineStroke = New("UIStroke", {
    ...
    -- 创建动画
    local Tween = TweenService:Create(Gradient, TweenInfo.new(7, Enum.EasingStyle.Linear, Enum.EasingDirection.In, -1), {
        Rotation = 360,
    })
    Tween:Play()
    
    return OutlineStroke, ShadowStroke, Gradient
end

-- 创建彩色文本函数
function Library:CreateColorfulText(TextLabel: TextLabel)
    local Gradient = New("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 0, 0)),
...
-- 创建白色按钮边框函数
function Library:AddWhiteOutline(Frame: GuiObject)
    local OutlineStroke = New("UIStroke", {
        Color = Color3.new(1, 1, 1),
        Thickness = 1.5,
        ZIndex = 2,
...
do 
    ...
    function Funcs:AddKeyPicker(Idx, Info) 
        ...
        Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject) 
            if Library.Unloaded then return end 
            if KeyPicker.Mode == "Always" or KeyPicker.Value == "Unknown" or KeyPicker.Value == "None" or 
               Picking or UserInputService:GetFocusedTextBox() then return end 
        ...
        Library:GiveSignal(UserInputService.InputEnded:Connect(function() 
            if Library.Unloaded then return end 
            if KeyPicker.Value == "Unknown" or KeyPicker.Value == "None" or 
               Picking or UserInputService:GetFocusedTextBox() then return end 
            KeyPicker:Update() 
        end)) 
    ...
    function Funcs:AddDependencyBox() 
        ...
        function Depbox:SetupDependencies(Dependencies) 
            for _, Dependency in Dependencies do 
                assert(typeof(Dependency) == "table", "依赖项应为表格。") 
...
function Library:CreateWindow(WindowInfo) 
    ...
    do 
        ...
        -- 使用彩色边框
        Library:CreateColorfulOutline(MainFrame) 
        ...
        -- 为标题添加彩色文本效果
        Library:CreateColorfulText(WindowTitle) 
    ...
    function Window:ChangeTitle(title) 
        assert(typeof(title) == "string", "期望字符串作为标题，但得到: " .. typeof(title)) 
    ...
    if Library.IsMobile then 
        local ToggleButton = Library:AddDraggableButton("显示/隐藏", function() -- 改为显示/隐藏
            Library:Toggle() 
        end, true) 
