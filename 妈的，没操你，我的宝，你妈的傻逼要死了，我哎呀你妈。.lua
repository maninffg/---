...
do
    ...
    function CustomImageManager.AddAsset(AssetName: string, RobloxAssetId: number, URL: string, ForceRedownload: boolean?)
        if CustomImageManagerAssets[AssetName] ~= nil then error(string.format("资源 %q 已存在", AssetName)) end
    ...
    function CustomImageManager.DownloadAsset(AssetName: string, ForceRedownload: boolean?)
        if not getcustomasset or not writefile or not isfile then return false, "缺少函数" end
        local AssetData = CustomImageManagerAssets[AssetName]
...
local Library = {
    ...
    Scheme = {
        ...
        OutlineColor = Color3.fromRGB(255, 255, 255), -- 改为白色
...
local function GetSchemeValue(Index)
    ...
    if AliasIndex and Library.Scheme[AliasIndex] ~= nil then
        warn(string.format("颜色方案值 %q 已弃用，请使用 %q 代替。", Index, AliasIndex))
...
function Library:ChangeCursorIcon(ImageId: string)
    ...
    assert(Icon, "图片必须是有效的 Roblox 资源或有效的 URL 或有效的 lucide 图标。")
...
function Library:ChangeCursorIconSize(Size: UDim2)
    assert(typeof(Size) == "UDim2", "需要 UDim2 类型。")
...
function Library:AddTooltip(InfoStr: string, DisabledInfoStr: string, HoverInstance: GuiObject)
    ...
    local function DoHover()
        if CurrentHoverInstance == HoverInstance or
            (CurrentMenu and Library:MouseIsOverFrame(CurrentMenu.Menu, Mouse)) or
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        if KeyPicker.Mode == "Press" then
            assert(ParentObj.Type == "Label", "按键选择器模式 'Press' 只能应用于标签。")
        ...
        function KeyPicker:Update()
            ...
            if KeyPicker.Mode == "开关" and ParentObj.Type == "Toggle" and ParentObj.Disabled then
        ...
        function KeyPicker:GetState()
            ...
            elseif KeyPicker.Mode == "按住" then
                ...
                if Key == "无" then return false end
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        local ColorMenu = Library:AddContextMenu(
            ...
            function() return { 0.5, Holder.AbsoluteSize.Y + 1.5 } end,
            1
        )
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:SetupDependencies(Dependencies)
            for _, Dependency in Dependencies do
                assert(typeof(Dependency) == "table", "依赖项应该是一个表格。")
...
function Library:Notify(...)
    ...
    function Data:Resize()
        if Title then
            local X, Y = Library:GetTextBounds(Title.Text, Title.FontFace, Title.TextSize, (NotificationArea.AbsoluteSize.X / Library.DPIScale) - 24)
            Title.Size = UDim2.fromOffset(0, Y)
            TitleX = X
        end
        if Desc then
            local X, Y = Library:GetTextBounds(Desc.Text, Desc.FontFace, Desc.TextSize, (NotificationArea.AbsoluteSize.X / Library.DPIScale) - 24)
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        titleGradient.Rotation = 10
        titleGradient.Parent = WindowTitle
        
        -- 为标题添加动画
        local titleTween = TweenService:Create(titleGradient, TweenInfo.new(7, Enum.EasingStyle.Linear, Enum.EasingDirection.In, -1), {
            Rotation = 360,
        })
        titleTween:Play()
    ...
    function Window:ChangeTitle(title)
        assert(typeof(title) == "string", "标题应为字符串，实际为: " .. typeof(title))
    ...
    Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject)
        ...
        if (typeof(Library.ToggleKeybind) == "table" and Library.ToggleKeybind.Type == "KeyPicker" and Input.KeyCode.Name == Library.ToggleKeybind.Value) or
            Input.KeyCode == Library.ToggleKeybind then
            Library.Toggle()
        end
    end))
